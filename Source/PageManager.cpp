...
namespace MeKernel
{
    ...
    PTEWrapper *PageManager::Request(Boolean Rw, Boolean User, Boolean ExecDisable)
    {
        PTEWrapper *PageTableEntry =
                reinterpret_cast<PTEWrapper*>(MeKernel::HAL::hal_alloc_page(sizeof(PTEWrapper), Rw, User));
        ...
        *PageTableEntry = PTEWrapper{Rw, User, ExecDisable, Details::create_page_wrapper(Rw, User)};
    ...
    {
        if (wrapper) {
            if (!Details::page_disable(wrapper->VirtualAddress()))
                return false;
